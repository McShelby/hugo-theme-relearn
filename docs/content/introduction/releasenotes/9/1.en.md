...
- {{% badge style="change" %}}Change{{% /badge %}} The Perfect Scrollbar library is no longer shipped with the theme. The scrollbars of the sidebar menu and the topbar flyouts are the ones of your browser now, styled by the theme - as is the scrollbar of the content, which the theme left alone before.

  For the menu only, the theme keeps drawing a scrollbar of its own where your browsers scrollbars take away space, as one of those would clip the menus border and its active entry. Whether they do is a matter of your operating systems and browsers settings - both of them know space taking and overlay scrollbars - and not of which browser you use.

  The scrollbars colors come from new [theme variant](configuration/branding/colors/#modifying-variants) variables, four for each of the three areas - the content, the menu and the topbar flyouts. `MAIN-SCROLLBAR-THUMB-color`, `MENU-SCROLLBAR-THUMB-color` and `TOPBAR-SCROLLBAR-THUMB-color` color the moving thumb, `MAIN-SCROLLBAR-TRACK-color`, `MENU-SCROLLBAR-TRACK-color` and `TOPBAR-SCROLLBAR-TRACK-color` the track it moves in. Each of them has a `-HOVER-` counterpart - like `MAIN-SCROLLBAR-THUMB-HOVER-color` or `MENU-SCROLLBAR-TRACK-HOVER-color` - for the respective hovered area.
